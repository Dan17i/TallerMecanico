...
/**
 * Servicio para generar reportes en formato PDF usando iText 8.
 * Contiene métodos para los 10 reportes del sistema MotorPlus.
 */
@Service
public class ReportePDFService {
     ...
     */
    public byte[] generarPDFSupervisionesPorMecanico(int idSupervisor) {
        List<SupervisionReporteDTO> supervisiones = reporteRepository.listarSupervisionesPorMecanico(idSupervisor);
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        ...
        try {
            ...
            if (!supervisiones.isEmpty()) {
                Paragraph supervisor = new Paragraph("Supervisor: " + supervisiones.get(0).getSupervisor())
                        .setFont(boldFont)
                        .setFontSize(11)
                        .setMarginBottom(10);
            ...
            Table resumen = new Table(1);
            resumen.setWidth(UnitValue.createPercentValue(100));
            resumen.addCell(createHeaderCellSingle("RESUMEN DE SUPERVISIONES", boldFont));
            resumen.addCell(createCell("Total Supervisiones: " + supervisiones.size(), font));
            document.add(resumen);
            document.add(new Paragraph("\n"));

            // Tabla de supervisiones
            Table table = new Table(new float[]{1, 3, 3, 2, 4});
            table.setWidth(UnitValue.createPercentValue(100));

            agregarHeaderCell(table, "ID SUP.", boldFont);
            agregarHeaderCell(table, "SUPERVISOR", boldFont);
            agregarHeaderCell(table, "SUPERVISADO", boldFont);
            agregarHeaderCell(table, "FECHA", boldFont);
            agregarHeaderCell(table, "OBSERVACIONES", boldFont);

            for (SupervisionReporteDTO sup : supervisiones) {
                table.addCell(createCell(String.valueOf(sup.getIdSupervision()), font));
                table.addCell(createCell(sup.getSupervisor(), font));
                table.addCell(createCell(sup.getMecanicoSupervisado(), font));
                table.addCell(createCell(sup.getFecha().format(DATE_FORMATTER), font));
                table.addCell(createCell(sup.getObservaciones() != null ? sup.getObservaciones() : "-", font));
            }
     ...
     * REPORTE (8): Servicios Realizados por Mecánico
     */
    public byte[] generarPDFServiciosPorMecanico(int idMecanico) {
        List<ServicioMecanicoReporteDTO> servicios = reporteRepository.listarServiciosPorMecanico(idMecanico);
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        ...
        try {
            ...
            agregarHeader(document, "SERVICIOS REALIZADOS POR MECÁNICO", boldFont);

            if (!servicios.isEmpty()) {
                Paragraph mecanico = new Paragraph("Mecánico: " + servicios.get(0).getNombreMecanico())
                        .setFont(boldFont)
                        .setFontSize(11)
                        .setMarginBottom(10);
                document.add(mecanico);
            }

            // Resumen
            Table resumen = new Table(1);
            resumen.setWidth(UnitValue.createPercentValue(100));
            resumen.addCell(createHeaderCellSingle("RESUMEN DE SERVICIOS", boldFont));
            resumen.addCell(createCell("Total Servicios: " + servicios.size(), font));
            document.add(resumen);
            document.add(new Paragraph("\n"));

            // Tabla de servicios
            Table table = new Table(new float[]{1, 3, 4, 2});
            table.setWidth(UnitValue.createPercentValue(100));

            agregarHeaderCell(table, "# ORDEN", boldFont);
            agregarHeaderCell(table, "MECÁNICO", boldFont);
            agregarHeaderCell(table, "SERVICIO", boldFont);
            agregarHeaderCell(table, "FECHA", boldFont);

            for (ServicioMecanicoReporteDTO serv : servicios) {
                table.addCell(createCell(String.valueOf(serv.getIdOrdenServicio()), font));
                table.addCell(createCell(serv.getNombreMecanico(), font));
                table.addCell(createCell(serv.getNombreServicio(), font));
                table.addCell(createCell(serv.getFecha().format(DATE_FORMATTER), font));
            }
